...
/**
 * The object is to verify that any given object matches a set of rules: ensure
 * that start and end are on one day AND ensure that start and end are not the
 * same AND ensure that duration is the difference between beginn and ende AND
 * ensure that duration is neither negativ nor zero AND ensure that end can not
 * be earlier than start (this is already ensured by the preceeding rules)
 * 
...
public final class standardRidingLessonVerifier implements lessonChangeObserver {
	...
	private void isStartAndEndOnOneDay() {
		if (!(begin.getMyDate().get(Calendar.DAY_OF_YEAR) == end.getMyDate()
				.get(Calendar.DAY_OF_YEAR))
				&& (begin.getMyDate().get(Calendar.YEAR) == end.getMyDate()
						.get(Calendar.YEAR))) {
